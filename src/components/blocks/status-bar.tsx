  ...
  (
    ...
    return (
      <HoverCard openDelay={0} closeDelay={0} open={isActive}>
        <HoverCardTrigger render={<div ref={ref} className="group relative flex h-full flex-1 cursor-pointer flex-col outline-none hover:opacity-80 focus-visible:opacity-80 focus-visible:ring-[2px] focus-visible:ring-ring/50 data-[aria-pressed=true]:opacity-80 rounded-full" onClick={() => handlers.onClick(index)} onFocus={() => handlers.onFocus(index)} onBlur={handlers.onBlur} onMouseEnter={() => handlers.onHoverStart(index)} onMouseLeave={handlers.onHoverEnd} onKeyDown={(e) => handlers.onKeyDown(e, index)} tabIndex={isLastItem && !isActive ? 0 : isActive ? 0 : -1} role="button" aria-label={`Day ${index + 1} status`} aria-pressed={isPinned} aria-expanded={isActive} data-slot="status-bar-item" />}><div className="flex h-full w-full flex-col overflow-hidden rounded-full">
                          {/* Render bar segments */}
